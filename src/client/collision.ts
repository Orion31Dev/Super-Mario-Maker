...
import { aabbQueue, Entity, rayQueue } from './entity';
import { blockActivationQueue } from './const';
...
function intersect(ray: Ray, aabb: AABB) {
  ...
  if (ray.origin.x >= aabb.x && ray.origin.x <= aabb.x + aabb.w && ray.origin.y >= aabb.y && ray.origin.y <= aabb.y + aabb.h)
    return ray.origin;
...
export function raycast(ray: Ray, colliders: Block[], triggerBlockActions: boolean) {
  var collisions = colliders
    .map(function (block) {
      ...
      if (result.intersection) {
        ...
        if (triggerBlockActions && !block.passable()) {
          if (result.distance === 0) {
            blockActivationQueue.push({ x: block.x, y: block.y });
          }
        }
      }
...
export function raycastEntities(ray: Ray, colliders: Entity[]) {
  var collisions = colliders
    .map(function (e) {
      let aabb = new AABB(e.x, e.y, e.width, e.height);
      var result = {
        collider: aabb,
        intersection: intersect(ray, aabb),
        distance: 0,
